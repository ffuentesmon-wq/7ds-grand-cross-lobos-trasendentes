...
def pedir_objetivo_runs():
    ...
    print(f"[Objetivo] El bot se detendrá automáticamente tras completar {objetivo} victorias.")
    return objetivo
            ...
            if fase_ganadora == "fase1":
                ...
                if localizar_en_emulador("miturno.png", confidence=0.70):
                    ...
                    if not ya_hizo_clic_lobo_f1:
                        print("[Fase 1] Buscando lobo_danable.png...")
                        pos_lobo = localizar_mas_a_la_derecha("lobo_danable.png", confidence=0.70)
                        if pos_lobo:
                            print(f"[Fase 1] ¡Lobo detectado! Aplicando desplazamiento...")
                            pos_lobo_desplazada = (pos_lobo[0] - 5, pos_lobo[1] + 120, pos_lobo[2], pos_lobo[3])
                            clic_en_zona_aleatoria(pos_lobo_desplazada)
            ...
            # ----------------------------------------------------
                ...
                if localizar_en_emulador("miturno.png", confidence=0.70):
                    ...
                    if not ya_hizo_clic_lobo_f2:
                        print("[Fase 2] Buscando lobo_danable_fase2.png...")
                        pos_lobo = localizar_mas_a_la_izquierda("lobo_danable_fase2.png", confidence=0.70)
                        if pos_lobo:
                            print(f"[Fase 2] ¡Lobo detectado! Aplicando desplazamiento...")
                            pos_lobo_desplazada = (pos_lobo[0] - 5, pos_lobo[1] + 120, pos_lobo[2], pos_lobo[3])
                            clic_en_zona_aleatoria(pos_lobo_desplazada)
